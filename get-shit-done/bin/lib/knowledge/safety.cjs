...
function getProtectedFiles(cwd) {
  ...
  const files = new Set();
  const addLines = (output) => { if (!output) return; for (const line of output.split('\n')) { const t = line.trim(); if (t) files.add(t); } };
...
function validateMergeReadiness(cwd, worktreePath, testCommand) {
  if (!worktreePath) return { ready: false, reason: 'No worktree path', testOutput: '' };
  try { if (!fs.statSync(worktreePath).isDirectory()) return { ready: false, reason: 'Not a directory', testOutput: '' }; }
  catch { return { ready: false, reason: 'Worktree does not exist', testOutput: '' }; }
  if (!testCommand) return { ready: false, reason: 'No test command', testOutput: '' };
  const parts = testCommand.split(/\s+/).filter(Boolean);
  const result = spawnSync(parts[0], parts.slice(1), { cwd: worktreePath, stdio: 'pipe', encoding: 'utf-8', timeout: 120000 });
