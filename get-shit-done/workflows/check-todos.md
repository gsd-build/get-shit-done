...
Extract from init JSON: `todo_count`, `todos`, `pending_dir`, `completed_dir`, `planning_base`.
...
If `{planning_base}/ROADMAP.md` exists:
...
**Work on it now:**
```bash
mv "${pending_dir}/[filename]" "${completed_dir}/"
```
