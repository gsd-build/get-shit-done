 ...
 *     Fix: remove GSD-managed hook blocks from config.toml and register the
 *     managed SessionStart hook in hooks.json instead.
 *
...
describe('#2760 defect 3 — Hooks AoT preservation across install/uninstall/reinstall', () => {
  ...
  test('preserves pre-existing [[hooks.SessionStart]] entries and adds GSD entry to hooks.json', () => {
    const userConfig = [
      '[[hooks.SessionStart]]',
    ...
    assert.ok(
      ...
      'GSD entry must not be emitted into config.toml hooks anymore: ' + JSON.stringify(commands)
    );

    const hooksJson = readCodexHooksJson(codexHome);
    const jsonCommands = hooksJson.hooks.SessionStart
      .flatMap((entry) => Array.isArray(entry.hooks) ? entry.hooks.map((h) => h.command) : []);
    assert.ok(
      jsonCommands.some((cmd) => typeof cmd === 'string' && /gsd-check-update\.js/.test(cmd)),
      'GSD entry must appear in hooks.json SessionStart array: ' + JSON.stringify(jsonCommands)
    );
  ...
  test('fresh install writes managed hook to hooks.json, not config.toml', () => {
    ...
    assert.ok(
      !parsed.hooks,
      'fresh install must not emit managed hooks into config.toml: ' + JSON.stringify(parsed.hooks)
    );

    const hooksJson = readCodexHooksJson(codexHome);
    assert.ok(hooksJson.hooks && Array.isArray(hooksJson.hooks.SessionStart), 'hooks.json has SessionStart array');
    const jsonCommands = hooksJson.hooks.SessionStart
      .flatMap((entry) => Array.isArray(entry.hooks) ? entry.hooks.map((h) => h.command) : []);
    assert.ok(
      jsonCommands.some((cmd) => typeof cmd === 'string' && /gsd-check-update\.js/.test(cmd)),
      'hooks.json SessionStart contains the managed GSD hook: ' + JSON.stringify(jsonCommands)
    );
  });
});
...
describe('#2760 CR4 finding 2 — Legacy flat [[hooks]] block migrates to hooks.json on reinstall', { concurrency: false }, () => {
  let tmpDir;
  let codexHome;
  ...
  test('pre-install legacy flat [[hooks]] gsd-check-update + user namespaced [[hooks.SessionStart]] -> post-install moves GSD hook to hooks.json', () => {
    ...
    // The user's namespaced TOML hook remains, but the legacy flat GSD block
    // is stripped and the managed GSD hook is registered in hooks.json.
    assert.ok(
      parsed.hooks && Array.isArray(parsed.hooks.SessionStart),
      'hooks.SessionStart must be an array-of-tables, got: '
        + (parsed.hooks ? typeof parsed.hooks.SessionStart : 'no hooks table')
    ...
    assert.ok(
      ...
      'GSD entry must not remain in config.toml hooks: ' + JSON.stringify(namespacedCommands)
    );
    ...
    // No duplicate gsd-check-update entries in hooks.json — exactly one managed entry.
    const hooksJson = readCodexHooksJson(codexHome);
    const jsonCommands = hooksJson.hooks.SessionStart
      .flatMap((entry) => Array.isArray(entry.hooks) ? entry.hooks.map((h) => h.command) : []);
    const gsdEntries = jsonCommands.filter(
      (cmd) => typeof cmd === 'string' && /gsd-check-update\.js/.test(cmd)
    );
    assert.equal(gsdEntries.length, 1,
      'exactly one hooks.json gsd-check-update entry after migration, got: ' + gsdEntries.length);
  });
});
...
describe('#2760 CR4 finding 1 — atomicWriteFileSync failure aborts install (post-write fatal)', { concurrency: false }, () => {
  ...
  test('install throws and never prints "Done!" when atomicWriteFileSync fails on configPath', () => {
    ...
    const configPath = path.join(codexHome, 'config.toml');
    // Only fault the final hook-feature atomic rename. Earlier writes to
    // config.toml happen via mergeCodexConfig (agent-block emit); the managed
    // hook itself now lives in hooks.json, so the final config write is the
    // second atomic rename to config.toml.
    let configRenameCount = 0;
    fs.renameSync = (src, dst) => {
      if (dst === configPath) {
        configRenameCount += 1;
        if (configRenameCount > 1) {
...
describe('#2760 CR5 finding 3 — migration removes managed flat AoT without disturbing user hooks', { concurrency: false }, () => {
  let tmpDir;
  let codexHome;
  ...
  test('user has [[hooks.AfterTool]] and legacy [hooks.SessionStart] -> post-install migrates user hook and uses hooks.json for GSD', () => {
    const userPlusLegacy = [
      '[[hooks.AfterTool]]',
      'command = "x"',
      '',
    ...
    assert.ok(
      parsed.hooks && Array.isArray(parsed.hooks.SessionStart),
      'legacy [hooks.SessionStart] must be migrated to namespaced AoT'
    );
    const sessionStartCommands = parsed.hooks.SessionStart.map((entry) => entry.command);
    assert.ok(
      sessionStartCommands.includes('y'),
      'user SessionStart command must be preserved: ' + JSON.stringify(sessionStartCommands)
    );
    for (const entry of parsed.hooks.SessionStart) {
      assert.equal(entry.event, undefined,
        'namespace [[hooks.SessionStart]] encodes the event: ' + JSON.stringify(entry));
    }
    ...
    assert.ok(!/gsd-check-update\.js/.test(after),
      'managed gsd-check-update hook must not be emitted into config.toml: ' + after);
    ...
    assert.ok(
      jsonCommands.some((cmd) => typeof cmd === 'string' && /gsd-check-update\.js/.test(cmd)),
      'managed GSD hook must be registered in hooks.json: ' + JSON.stringify(jsonCommands)
    );
  });

  test('user has [[hooks.AfterTool]] and legacy flat GSD hook -> post-install moves managed hook to hooks.json', () => {
    // The user-owned namespaced AoT entry stays in config.toml. The legacy
    // managed GSD flat hook is stripped from config.toml and registered in
    // hooks.json so config.toml does not mix managed hook shapes.
    const userPlusLegacy = [
      '[[hooks.AfterTool]]',
      'command = "x"',
      '',
    ...
    // No managed GSD hook remains in config.toml, and no flat top-level
    // [[hooks]] AoT may remain.
    assert.ok(
      !Array.isArray(parsed.hooks) || parsed.hooks.length === 0,
      'no flat top-level [[hooks]] AoT entries may remain after migration: ' +
        JSON.stringify(parsed.hooks)
    ...
    assert.ok(!/gsd-check-update\.js/.test(after),
      'managed gsd-check-update hook must be removed from config.toml: ' + after);
