...
function writeCodexHooks(codexHome, content) {
  fs.mkdirSync(codexHome, { recursive: true });
  fs.writeFileSync(path.join(codexHome, 'hooks.json'), content, 'utf8');
}
...
describe('generateCodexConfigBlock', () => {
  ...
  test('does not include feature flags or the broken [[agents]] shape', () => {
    const result = generateCodexConfigBlock(agents);
    ...
    assert.ok(result.match(/^\[agents\.gsd-/m), 'uses [agents.gsd-*] sections');
    // Should not have bare [agents] table header either.
    assert.ok(!result.match(/^\[agents\]\s*$/m), 'no bare [agents] table');
    assert.ok(!result.match(/^\[\[agents\]\]\s*$/m), 'no broken [[agents]] headers');
    assert.ok(!result.includes('max_threads'), 'no max_threads');
    assert.ok(!result.includes('max_depth'), 'no max_depth');
  });

  test('emits [agents.gsd-*] subtables for current Codex schema', () => {
    const result = generateCodexConfigBlock(agents);
    assert.strictEqual((result.match(/^\[agents\.gsd-/gm) || []).length, 2, 'one [agents.gsd-*] header per agent');
    assert.ok(result.includes('[agents.gsd-executor]'), 'executor uses subtable header');
    assert.ok(result.includes('[agents.gsd-planner]'), 'planner uses subtable header');
  });

  test('block contains no broken [[agents]] tables', () => {
    const result = generateCodexConfigBlock(agents);
    assert.strictEqual((result.match(/^\[\[agents\]\]\s*$/gm) || []).length, 0, 'no [[agents]] headers remain');
  });
...
describe('stripGsdFromCodexConfig', () => {
  ...
  test('removes broken [[agents]] array-of-tables entries whose name is gsd-*', () => {
  ...
  test('#2645: handles mixed legacy + new shapes and multiple user/gsd entries in one file', () => {
      // Multiple GSD entries (current subtable + broken array-of-tables) interleaved
    ...
    assert.ok(result.includes('user legacy'), 'preserves user legacy body');
      assert.ok(result.includes('name = "my-helper"'), 'preserves user broken-form [[agents]]');
      assert.ok(result.includes('name = "another-user"'), 'preserves second user broken-form [[agents]]');
...
describe('mergeCodexConfig', () => {
  ...
  test('case 3 strips existing [agents.gsd-*] sections before appending fresh block', () => {
    ...
    const beforeMarker = markerIndex === -1 ? content : content.slice(0, markerIndex);
    const legacyGsdAgentCount = (beforeMarker.match(/^\[agents\.gsd-executor\]\s*$/gm) || []).length;
    const managedAgentCount = (content.match(/^\[agents\.gsd-executor\]\s*$/gm) || []).length;
    const markerCount = (content.match(new RegExp(GSD_CODEX_MARKER.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'g')) || []).length;
    ...
    assert.strictEqual(managedAgentCount, 1, 'keeps exactly one managed GSD [agents.gsd-*] entry');
  ...
  test('case 2 strips leaked GSD-managed sections above marker in CRLF files', () => {
    ...
    assert.strictEqual(countMatches(content, /^\[agents\.gsd-executor\]\s*$/gm), 1, 'keeps one managed [agents.gsd-*] section');
    assert.strictEqual(countMatches(content, /^\[\[agents\]\]\s*$/gm), 0, 'removes broken [[agents]] sections');
  ...
  test('case 2 preserves user-authored [agents] tables while stripping leaked GSD sections in CRLF files', () => {
    ...
    assert.strictEqual(countMatches(beforeMarker, /^\[agents\.gsd-executor\]\s*$/gm), 0, 'removes leaked GSD agent section above marker');
    assert.strictEqual(countMatches(content, /^\[\[agents\]\]\s*$/gm), 0, 'no broken [[agents]] sections remain');
    assert.strictEqual(countMatches(content, /^\[agents\.gsd-executor\]\s*$/gm), 1, 'keeps one managed agent entry in the GSD block');
    assertUsesOnlyEol(content, '\r\n');
  });
...
describe('Codex install hook configuration (e2e)', () => {
  ...
  test('Codex install copies hook file that is referenced in config.toml (#2153)', () => {
    // Regression test: Codex install writes the update hook registration into
    // hooks.json and must also copy the hook file to ~/$CODEX_HOME/hooks/
    runCodexInstall(codexHome);

    const hooks = readCodexHooks(codexHome);
    assert.ok(hooks.hooks.SessionStart, 'hooks.json contains SessionStart hooks');
    const commands = hooks.hooks.SessionStart.flatMap(entry => (entry.hooks || []).map(h => h.command));
    assert.ok(commands.some(cmd => cmd.includes('gsd-check-update.js')), 'hooks.json references gsd-check-update.js');
  ...
  test('fresh CODEX_HOME enables codex_hooks without draft root defaults', () => {
    ...
    assert.ok(!content.includes('[[hooks]]'), 'does not write invalid [[hooks]] block');
    assert.strictEqual(countMatches(content, /^codex_hooks = true$/gm), 1, 'writes one codex_hooks key');
    const commands = hooks.hooks.SessionStart.flatMap(entry => (entry.hooks || []).map(h => h.command));
    assert.strictEqual(commands.filter(cmd => cmd.includes('gsd-check-update.js')).length, 1, 'writes one GSD update hook to hooks.json');
  ...
  test('existing LF config without [features] gets one features block and preserves user content', () => {
    writeCodexConfig(codexHome, [
      ...
      '',
      '[hooks]',
      'SessionStart = [{ type = "command", command = "echo custom" }]',
      '',
    ].join('\n'));
    ...
    assert.ok(content.includes('SessionStart = [{ type = "command", command = "echo custom" }]'), 'preserves user config.toml hook syntax');
    const commands = hooks.hooks.SessionStart.flatMap(entry => (entry.hooks || []).map(h => h.command));
    assert.strictEqual(commands.filter(cmd => cmd === 'echo custom').length, 0, 'does not mirror config.toml hooks into hooks.json');
    assert.strictEqual(commands.filter(cmd => cmd.includes('gsd-check-update.js')).length, 1, 'adds one GSD update hook to hooks.json');
    assertNoDraftRootKeys(content);
  });
  ...
  test('existing [features] at EOF without trailing newline is updated in place', () => {
    ...
    assert.ok(content.indexOf('codex_hooks = true') < content.indexOf('[agents.gsd-executor]'), 'keeps codex_hooks before the first managed [agents.gsd-*] entry');
    assertNoDraftRootKeys(content);
  });
  ...
  test('existing empty [features] and codex_hooks = false are normalized and remain idempotent', () => {
    ...
    writeCodexHooks(codexHome, JSON.stringify({
      hooks: {
        SessionStart: [
          {
            hooks: [
    ...
    const commands = hooks.hooks.SessionStart.flatMap(entry => (entry.hooks || []).map(h => h.command));
    assert.ok(commands.includes('echo custom'), 'preserves custom hook in hooks.json');
    assert.strictEqual(commands.filter(cmd => cmd.includes('gsd-check-update.js')).length, 1, 'does not duplicate GSD update hook');
    assertNoDraftRootKeys(content);
  });
  ...
  test('quoted [features] headers are recognized as the existing features table', () => {
    ...
    const commands = hooks.hooks.SessionStart.flatMap(entry => (entry.hooks || []).map(h => h.command));
    assert.strictEqual(commands.filter(cmd => cmd.includes('gsd-check-update.js')).length, 1, 'keeps one GSD update hook');
    assertNoDraftRootKeys(content);
  });
  ...
  test('quoted table headers containing # are parsed without treating # as a comment start', () => {
    ...
    assert.strictEqual(countMatches(content, /^codex_hooks = true$/gm), 1, 'adds one codex_hooks key');
    const commands = hooks.hooks.SessionStart.flatMap(entry => (entry.hooks || []).map(h => h.command));
    assert.strictEqual(commands.filter(cmd => cmd.includes('gsd-check-update.js')).length, 1, 'remains idempotent for the GSD hook entry');
    assertNoDraftRootKeys(content);
  });
  ...
  test('existing dotted features config stays dotted and does not grow a [features] table', () => {
    ...
    const commands = hooks.hooks.SessionStart.flatMap(entry => (entry.hooks || []).map(h => h.command));
    assert.strictEqual(commands.filter(cmd => cmd.includes('gsd-check-update.js')).length, 1, 'adds one GSD update hook for dotted codex_hooks and remains idempotent');
    assertNoDraftRootKeys(content);
  });
  ...
  test('root inline-table features assignments are left untouched without appending invalid dotted keys or hooks', () => {
    ...
    assert.ok(!fs.existsSync(path.join(codexHome, 'hooks.json')), 'does not add hooks.json when codex_hooks cannot be enabled safely');
  ...
  test('quoted dotted codex_hooks keys stay dotted and are normalized without duplication', () => {
    ...
    const commands = hooks.hooks.SessionStart.flatMap(entry => (entry.hooks || []).map(h => h.command));
    assert.strictEqual(commands.filter(cmd => cmd.includes('gsd-check-update.js')).length, 1, 'adds one GSD update hook for quoted dotted codex_hooks and remains idempotent');
    assertNoDraftRootKeys(content);
  });
  ...
  test('multiline strings inside [features] do not create fake tables or fake codex_hooks matches', () => {
    writeCodexConfig(codexHome, [
      ...
      '',
      '[hooks]',
      'AfterCommand = [{ type = "command", command = "echo custom-after-command" }]',
      '',
    ].join('\n'));
  ...
  test('existing CRLF codex_hooks = true stays single and preserves non-GSD hooks', () => {
    ...
    assert.ok(content.includes('other_feature = true'), 'preserves other feature keys');
    assert.strictEqual(countMatches(content, /echo custom-after-command/g), 1, 'preserves non-GSD config hook exactly once');
    const commands = hooks.hooks.SessionStart.flatMap(entry => (entry.hooks || []).map(h => h.command));
    assert.strictEqual(commands.filter(cmd => cmd.includes('gsd-check-update.js')).length, 1, 'keeps one GSD update hook');
    assertUsesOnlyEol(content, '\r\n');
    assertNoDraftRootKeys(content);
  ...
  test('codex_hooks = true with an inline comment is treated as enabled for hook installation', () => {
    ...
    assert.ok(content.includes('other_feature = true'), 'preserves other feature keys');
    const commands = hooks.hooks.SessionStart.flatMap(entry => (entry.hooks || []).map(h => h.command));
    assert.strictEqual(commands.filter(cmd => cmd.includes('gsd-check-update.js')).length, 1, 'adds the GSD update hook once');
    assertNoDraftRootKeys(content);
  });
  ...
  test('mixed-EOL configs use the first newline style for inserted Codex content', () => {
    ...
    const commands = hooks.hooks.SessionStart.flatMap(entry => (entry.hooks || []).map(h => h.command));
    assert.strictEqual(commands.filter(cmd => cmd.includes('gsd-check-update.js')).length, 1, 'writes the GSD hook entry to hooks.json');
...
describe('Codex uninstall symmetry for hook-enabled configs', () => {
  ...
  test('install then uninstall removes [features].codex_hooks while preserving other feature keys, comments, hooks, and CRLF', () => {
    ...
    assert.strictEqual(countMatches(cleaned, /echo custom-after-command/g), 1, 'preserves non-GSD config hooks');
    assert.strictEqual(countMatches(cleaned, /gsd-check-update\.js/g), 0, 'config.toml no longer contains the GSD update hook');
  ...
  test('install then uninstall removes only GSD hooks from hooks.json', () => {
    runCodexInstall(codexHome);
    ...
    writeCodexHooks(codexHome, JSON.stringify({
      hooks: {
        SessionStart: [
          {
            hooks: [
              { type: 'command', command: 'node /tmp/gsd-check-update.js' },
        ...
        PostToolUse: [
          {
            hooks: [
              { type: 'command', command: 'node /tmp/gsd-context-monitor.js' },
    ...
    const hooks = readCodexHooks(codexHome);
    const sessionCommands = hooks.hooks.SessionStart.flatMap(entry => (entry.hooks || []).map(h => h.command));
    const postCommands = hooks.hooks.PostToolUse.flatMap(entry => (entry.hooks || []).map(h => h.command));

    assert.ok(!sessionCommands.some(cmd => cmd.includes('gsd-check-update')), 'removes GSD SessionStart hook');
    assert.ok(!postCommands.some(cmd => cmd.includes('gsd-context-monitor')), 'removes GSD PostToolUse hook');
    assert.ok(sessionCommands.includes('echo keep-me'), 'preserves user SessionStart hook');
    assert.ok(postCommands.includes('echo keep-post'), 'preserves user PostToolUse hook');
  });
});
