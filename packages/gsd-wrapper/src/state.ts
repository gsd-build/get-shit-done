...
/**
 * Raw output structure from gsd-tools state --json
 * The state command returns config + state_raw (the full STATE.md content)
 */
interface RawStateOutput {
 ...
 */
function parseProgress(stateRaw: string): { total: number; completed: number } {
 ...
 */
function parsePhase(stateRaw: string): string {
 ...
 */
function parsePlan(stateRaw: string): string {
 ...
 */
function parseStatus(stateRaw: string): ProjectState['status'] {
  const frontmatterMatch = stateRaw.match(/^---\n([\s\S]*?)\n---/);
  if (!frontmatterMatch) return 'pending';
 ...
 */
function parseMilestone(stateRaw: string): string | undefined {
  const frontmatterMatch = stateRaw.match(/^---\n([\s\S]*?)\n---/);
 ...
 */
function parseLastActivity(stateRaw: string): string | undefined {
...
export async function getState(cwd: string): Promise<GsdResult<ProjectState>> {
  const result = await execGsdTools<RawStateOutput>(['state', '--json'], cwd);
 ...
 */
export async function getProgress(cwd: string): Promise<number | null> {
