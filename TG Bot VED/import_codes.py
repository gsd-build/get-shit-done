...
# === НАСТРОЙКИ ===
PDF_PATH = Path(
    r"C:\Users\Александр Бесчастный\OneDrive\Рабочий стол\git\MyfirsProject\ru.84_2022_21.09.2025.pdf"
)
DB_PATH = Path(os.getenv("DB_PATH", "bot.db"))

# 1) ловим формат с пробелами: 8408 10 110 0  (4 + 2 + 3 + 1 = 10)
CODE_SPACED_RE = re.compile(r"\b(84(?:08|18))\s*(\d{2})\s*(\d{3})\s*(\d)\b")

# 2) ловим слитный формат: 8408101100
...
def extract_codes_from_pdf(pdf_path: Path) -> dict[str, str]:
    ...
    # на всякий случай: оставляем только строго 8408/8418 и 10 цифр
    cleaned = {
        c: t
