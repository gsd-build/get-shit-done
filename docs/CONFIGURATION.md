...
| `workflow.human_verify_mode` | string | `'mid-flight'` | Controls how the planner emits human verification work. `'mid-flight'` (default) emits `<task type="checkpoint:human-verify">` tasks that halt the executor for confirmation — each halt costs a full executor cold-start (CLAUDE.md, MEMORY.md, STATE.md, plan re-read) because subagent context is discarded across the pause. `'end-of-phase'` (issue #3309) suppresses those tasks entirely; the planner embeds verification details into `<verify><human-check>` blocks on `auto` tasks, and the verifier consolidates them into the existing HUMAN-UAT.md flow at end-of-phase. Use `end-of-phase` when checkpoint round-trip cost dominates your token bill. `checkpoint:decision` and `checkpoint:human-action` are unaffected. |
