...
| `workflow.text_mode` | boolean | `false` | Replaces AskUserQuestion TUI menus with plain-text numbered lists. Required for Claude Code remote sessions (`/rc` mode) where TUI menus don't render. Codex should prefer `request_user_input` with top-level `questions[]` when available; use text mode only as explicit fallback or when the runtime tool is unavailable or rejected. Can also be set per-session with `--text` flag on discuss-phase. Added in v1.28 |
