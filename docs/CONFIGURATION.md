...
| `workflow.human_verify_mode` | string | `'end-of-phase'` | Controls how the planner emits human verification work. `'end-of-phase'` (default since #3309) suppresses `<task type="checkpoint:human-verify">` task emission — the planner embeds verification details into `<verify><human-check>` blocks on `auto` tasks and the verifier consolidates them into the existing HUMAN-UAT.md flow at end-of-phase. `'mid-flight'` restores pre-#3309 behavior where the executor halts at each `checkpoint:human-verify` task; each halt costs a full executor cold-start (CLAUDE.md, MEMORY.md, STATE.md, plan re-read on respawn) because subagent context is discarded across the pause. The default was flipped because that cold-start cost was reported as "tens of thousands of tokens" per round-trip on real projects. `checkpoint:decision` and `checkpoint:human-action` are unaffected by either value. |
