...
const banner = '\n' +
  ...
  '  development system for Claude Code, OpenCode, Gemini, Codex, and GitHub Copilot CLI by TÂCHES.\n';
...
function getCommitAttribution(runtime) {
  ...
  if (runtime === 'opencode') {
    ...
    // Codex and Copilot CLI currently have no attribution setting equivalent
    result = undefined;
  }
...
GitHub Copilot CLI skills mode:
- This skill is activated when you ask Copilot to run \`${skillName}\` or reference it by name.
- Treat any user text following the skill invocation as \`{{GSD_ARGS}}\`.
- If no arguments are present, treat \`{{GSD_ARGS}}\` as empty.

Subagent compatibility:
- Any \`Task(...)\` pattern in referenced workflow docs is legacy orchestration syntax.
- In GitHub Copilot CLI, use the built-in task tool with appropriate agent_type to spawn subagents.
- Treat legacy \`subagent_type\` names as hints for the agent_type parameter.
...
// Alias used by both Codex and Copilot skill listing
function listSkillNames(skillsDir, prefix = 'gsd-') {
  return listCodexSkillNames(skillsDir, prefix);
}
...
function copyCommandsAsCopilotSkills(srcDir, skillsDir, prefix, pathPrefix, runtime) {
  ...
  // Remove previous GSD Copilot skills to avoid stale command skills.
  ...
  function recurse(currentSrcDir, currentPrefix) {
    ...
    for (const entry of entries) {
      ...
      const globalClaudeRegex = /~\/\.claude\//g;
      const localClaudeRegex = /\.\/\.claude\//g;
      const copilotDirRegex = /~\/\.copilot\//g;
      content = content.replace(globalClaudeRegex, pathPrefix);
      content = content.replace(localClaudeRegex, `./${getDirName(runtime)}/`);
      content = content.replace(copilotDirRegex, pathPrefix);
      content = processAttribution(content, getCommitAttribution(runtime));
...
function uninstall(isGlobal, runtime = 'claude') {
  ...
  if (isOpencode) {
    ...
    // Copilot CLI: remove skills/gsd-*/SKILL.md skill directories
    const skillsDir = path.join(targetDir, 'skills');
    if (fs.existsSync(skillsDir)) {
      let skillCount = 0;
      const entries = fs.readdirSync(skillsDir, { withFileTypes: true });
...
function install(isGlobal, runtime = 'claude') {
  ...
  // OpenCode uses command/ (flat), Codex uses skills/, Copilot uses skills/, Claude/Gemini use commands/gsd/
  if (isOpencode) {
    ...
    const installedSkillNames = listSkillNames(skillsDir);
    if (installedSkillNames.length > 0) {
      console.log(`  ${green}✓${reset} Installed ${installedSkillNames.length} skills to skills/`);
    } else {
      failures.push('skills/gsd-*');
...
function promptRuntime(callback) {
  ...
  ${cyan}5${reset}) GitHub Copilot   ${dim}(~/.copilot)${reset}
  ${cyan}6${reset}) All
`);
