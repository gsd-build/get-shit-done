...
- **`/gsd-reapply-patches` Step 5 verifier now resolves at runtime** — `verify-reapply-patches.cjs` (added in #2972 as the deterministic gate that closes #2969's `verified: yes`-without-checking failure) shipped under the top-level `scripts/` directory, but the installer copies `get-shit-done/` recursively to `${configDir}/get-shit-done/` and does NOT copy `scripts/`. As a result, every fresh install hit `Cannot find module …/scripts/verify-reapply-patches.cjs` on Step 5 — the `${GSD_HOME}/scripts/` path simply did not exist on disk. The whole rationale for #2972 (move verification out of LLM-driven prose into a deterministic script) was undone if the script wasn't reachable. Fix: move the script to `get-shit-done/bin/verify-reapply-patches.cjs` (which IS installed, same pattern as `gsd-tools.cjs` and other runtime-needed bin scripts) and update `reapply-patches.md` Step 5 to invoke `${GSD_HOME}/get-shit-done/bin/verify-reapply-patches.cjs`. The existing `tests/bug-2969-verify-reapply-patches.test.cjs` SCRIPT path is updated to match. New regression `tests/bug-2994-verify-reapply-patches-installed-path.test.cjs` (5 tests across two suites) parses `reapply-patches.md` into structured invocation records and asserts: (1) the script exists at the runtime-installed path, (2) the legacy `scripts/` location is empty, (3) the script is `require()`-able and exposes its `REASON` enum, (4) every `node "${GSD_HOME}/…"` reference in the workflow lives under `get-shit-done/` (the installed tree), (5) the verifier-specific reference matches the new path exactly. (#2994)
