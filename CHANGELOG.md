...
- **Installer now rebuilds a stale `gsd-sdk` on upgrade** — v1.37.1 installed `@gsd-build/sdk@0.1.0` globally, which lacked the `query` subcommand every `/gsd-*` skill depends on. PR #2386 fixed fresh installs by building from in-repo source, but the rebuild was skipped on upgrade because `which gsd-sdk` still found the stale binary. `installSdkIfNeeded()` now probes `gsd-sdk --help` for the `query` capability and rebuilds from source if it's missing; missing binary, timeout, and non-zero exit all fall through to the rebuild path. `--sdk` and `--no-sdk` semantics unchanged (#2414)
