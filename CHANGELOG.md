...
- **`release-sdk` hotfix push rejected with "refusing to allow a GitHub App to create or update workflow … without `workflows` permission"** — the cherry-pick loop in `Prepare hotfix branch` happily picked commits modifying `.github/workflows/*`, then the trailing `git push` was rejected by GitHub because the default `GITHUB_TOKEN` lacks the `workflow` scope. The whole hotfix run aborted with no clear signal which commit was the culprit. The loop now pre-skips any candidate commit whose `git diff-tree` lists files under `.github/workflows/`, surfacing each skipped commit via a `::warning::` annotation AND a dedicated `WORKFLOW_SKIPPED` section in the run summary so operators don't ship a hotfix that silently dropped a workflow-file fix. Workflow-file fixes intended for a hotfix must be applied to the hotfix branch via a token with `workflow` scope, or landed on `main` and re-cut. Regression covered by `tests/bug-2980-skip-workflow-file-cherrypicks.test.cjs`. (#2980)
