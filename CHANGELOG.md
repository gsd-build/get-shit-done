...
- **SDK `config-set`/`config-get` and `init` no longer echo plaintext API keys** — four response paths emitted the raw value of secret config keys (`brave_search`, `firecrawl`, `exa_search`) into machine-readable JSON. The masking spec at `get-shit-done/bin/lib/secrets.cjs` (`SECRET_CONFIG_KEYS`, `maskSecret`) was correctly applied in `cmdConfigSet`/`cmdConfigGet`, but the CJS→TS port at `sdk/src/query/config-mutation.ts:240,243` and `sdk/src/query/config-query.ts:122,128,132` dropped the masking layer entirely — `gsd-sdk query config-set brave_search BSA-XYZ…` echoed the plaintext credential into stdout, transcripts, CI logs, and any tooling that captured the response. An adjacent leak existed in `sdk/src/query/init.ts:673-675` and the CJS analogue at `get-shit-done/bin/lib/init.cjs:728-730`: the init bundle passed `config.brave_search` raw, leaking the API key whenever the user had stored one (booleans pass through unchanged). Fix: a new `sdk/src/query/secrets.ts` (TS port of `secrets.cjs`, exact CJS parity, exercised by `secrets.test.ts` against the CJS module) is wired into all four call sites. `config-set` masks both `value` and `previousValue` in its response while leaving the on-disk plaintext intact (the key is still usable). `config-get` masks the read response. `--default` user-supplied values pass through unmasked (those are the user's own input, not stored secrets). Init bundles only mask string values, preserving the boolean availability-flag contract. Regression covered by 17 tests in `sdk/src/query/secrets.test.ts` (including CJS-parity assertions on every short-string and tail-leak case), 5 in `config-mutation.test.ts` (`describe('configSet secret masking (#2997)')` block, including the on-disk-plaintext-preserved invariant), and 4 in `config-query.test.ts`. (#2997)
