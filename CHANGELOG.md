...
- **`gsd-code-fixer` worktree setup no longer fails when current branch is already checked out** — the agent definition called `git worktree add "$wt" "$branch"` where `$branch` was the user's currently-checked-out branch in the main repo. Git refuses to check out the same branch in two worktrees by default, so the agent failed before applying any review fixes. This is the next-layer failure after #2686 (foreground/background race) and #2839 (transactional cleanup) — the isolation strategy was correct in design but blocked by git's same-branch protection. Fix: create a NEW branch `gsd-reviewfix/${padded_phase}-$$` and attach the worktree to it via `git worktree add -b "$reviewfix_branch" "$wt" "$branch"`. Commits inside the worktree advance the temp branch. The cleanup tail (now four-step) fast-forwards `$branch` to `$reviewfix_branch` from the main repo via `git -C "$main_repo" merge --ff-only "$reviewfix_branch"` so the user's branch captures the agent's commits, then removes the worktree, deletes the temp branch only if the fast-forward succeeded (preserving it for manual merge on divergence), then drops the recovery sentinel. The recovery sentinel JSON now records `reviewfix_branch` alongside `worktree_path` so a re-run after interruption can clean the orphan branch as well as the orphan worktree (#2839 recovery property preserved). Regression covered by `tests/bug-2990-code-fixer-worktree-branch.test.cjs` (6 tests across two suites): parses the agent .md into structured `git worktree add` invocation records, skips occurrences inside markdown inline-code or bash comments (those are citations of the OLD pattern, not executable), and asserts (1) every executable invocation uses `-b "$reviewfix_branch"`, (2) the cleanup documents `merge --ff-only "$reviewfix_branch"`, (3) the cleanup documents `branch -D "$reviewfix_branch"` gated on fast-forward success, (4) the recovery sentinel includes `reviewfix_branch`. (#2990)
