...
- **`/gsd-profile-user --refresh` writes `dev-preferences.md` to the skills/ location, not the legacy `commands/gsd/`** — the v1.39.0 install summary claims the legacy `~/.claude/commands/gsd/` directory has been removed in favor of skills-only architecture, but the `cmdGenerateDevPreferences` writer at `get-shit-done/bin/lib/profile-output.cjs:781` hardcoded the legacy path as the fallback when no `--output` flag was passed. The `/gsd-profile-user` workflow does not pass `--output`, so every refresh deterministically re-created the legacy directory. Missed in PR #1540's commands→skills migration because dev-preferences is a runtime-generated user artifact, not a GSD-shipped command file. Fix: (1) writer default targets `~/.claude/skills/gsd-dev-preferences/SKILL.md`; (2) `profile-user.md` Display message and artifact list reference the new path; (3) `bin/install.js` ships a new helper `migrateLegacyDevPreferencesToSkill(targetDir, savedArtifacts)` invoked at all 5 skills-aware install branches (Antigravity, Hermes ×2, Qwen ×2, Claude global) that copies any preserved legacy `dev-preferences.md` into the skills location during install. Migration is a no-op if `SKILL.md` already exists at the new location, so user-customized skill content is never clobbered. Regression covered by `tests/bug-2973-profile-user-skills-path.test.cjs` (6 tests across three suites): runs the writer in a subprocess (because `core.cjs:output()` uses `fs.writeSync(1, ...)` which bypasses in-process stubbing) and asserts `command_path` is the skills path, the file exists on disk, the legacy path is not created, the workflow Display message and artifact list reference the new path, the migration helper is exported, migration writes when no SKILL.md exists, migration is a no-op when one already does. (#2973)
