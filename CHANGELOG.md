...
- **`config-new-project` now propagates nested keys from `~/.gsd/defaults.json`** — `configNewProject` previously merged the global defaults file at the top level only; each nested section (`git`, `workflow`, `hooks`, `agent_skills`, `features`) was rebuilt with a two-way spread of hardcoded defaults and `userChoices`, silently dropping every `globalDefaults.<section>` key. As a result, machine-wide preferences like `workflow.code_review_depth: "deep"` or `git.branching_strategy: "milestone"` saved in `~/.gsd/defaults.json` were replaced by the hardcoded defaults in every new project, contradicting the documented `hardcoded <- globalDefaults <- userChoices` precedence. Each nested section now spreads `globalDefaults.<section>` between the hardcoded defaults and `userChoices`, restoring the documented precedence. Regression tests added in `sdk/src/query/config-mutation.test.ts` cover `workflow.*`, `git.*`, `hooks.*`, `features.*`, and the three-way precedence rule. Closes #2673.
