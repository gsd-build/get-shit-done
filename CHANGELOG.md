...
- **Managed JS hooks now resolve under GUI/minimal-PATH runtimes** — installer-emitted hook commands started with bare `node` (`node "<HOME>/.gemini/hooks/gsd-check-update.js"`), which works from an interactive shell where nvm/Homebrew/Volta has Node on PATH, but fails in GUI-launched runtimes (Antigravity, Gemini CLI from Finder, etc.) that start with the minimal POSIX PATH `/usr/bin:/bin:/usr/sbin:/sbin`. On most macOS hosts `/usr/bin/node` does not exist, so every managed JS hook (gsd-check-update, gsd-statusline, gsd-context-monitor, gsd-prompt-guard, gsd-read-guard, gsd-read-injection-scanner, gsd-workflow-guard) failed with `/bin/sh: node: command not found` (exit 127) — silently disabling update checks, statusline, and security guards even though the install reported success. Reporter validated this end-to-end across Gemini and Antigravity, fresh + legacy-seed scenarios. Fix: a new `resolveNodeRunner()` helper returns `process.execPath` (the absolute path of the Node binary running the installer — the version the user just installed under) forward-slash-normalized and double-quoted, used as the runner for every `.js` hook in both `buildHookCommand()` (global installs) and the local-install code paths (statusline, update, context-monitor, prompt-guard, read-guard, read-injection-scanner, workflow-guard). `.sh` hooks keep bare `bash` because `/bin/bash` is in the POSIX standard PATH and always resolves. Regression covered by `tests/bug-2979-hook-absolute-node.test.cjs` (8 tests across three suites): asserts `resolveNodeRunner()` returns a quoted, absolute, forward-slash-normalized path whose basename is `node`/`node.exe`; `buildHookCommand()` for `.js` emits absolute node both for default and `portableHooks` modes; `.sh` runner is still bare `bash`. Tests parse emitted commands into `{ runner, hookPath }` records (typed-IR style) rather than regex on raw text. (#2979)
