...
- **Community bash hooks use `#!/usr/bin/env bash` for NixOS/Alpine portability** — `gsd-phase-boundary.sh`, `gsd-session-state.sh`, and `gsd-validate-commit.sh` shipped with `#!/bin/bash` as their shebang. On NixOS and minimal Alpine container images `/bin/bash` does not exist (bash lives at `/usr/bin/bash`), so direct invocation of the hook files failed with "bad interpreter: /bin/bash: no such file or directory". Changed to `#!/usr/bin/env bash` to use PATH lookup, matching the convention in `scripts/*.sh`. The default install path (`bash "<hook-path>"` in settings.json) is unaffected. (#3104)
