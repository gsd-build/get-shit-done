...
- **`/gsd-sync-github` — one-way mirror of GSD planning state to GitHub Issues + Milestones** — New slash command + PostToolUse hook + helper library that project the `.planning/` state onto GitHub. Modes: `--dry-run` (default), `--apply`, `--seed-historical` (creates a single closed umbrella issue per milestone with shipped phases as a checklist), `--repair` (rebuild registry from GitHub state). Opt-in per project via `.planning/config.json#github_sync.enabled` (default `false`). Sync never blocks GSD actions — preflight (gh auth, issues-enabled, rate limit) failures degrade gracefully and log to `.planning/.github-sync.log`. Idempotent. Existing GSD skills are unmodified — the hook observes from outside. Uses Node.js for JSON parsing (no `jq` dependency, matching `hooks/gsd-phase-boundary.sh`). Field-tested in a 35-phase real project. (#2191)
