...
- **`gsd-check-update-worker.js` now resolves `npm` correctly on Windows** — `execFileSync('npm', ...)` was called without `shell: true`. Node's `execFileSync` does not spawn a shell and does not apply PATHEXT resolution, so on Windows it looked for a literal file named `npm` (which does not exist — npm ships as `npm.cmd`) and threw ENOENT. The error was silently swallowed, `latest` stayed `null`, and the `⬆ /gsd-update` status-line indicator never rendered for Windows users. Adding `shell: true` lets cmd.exe resolve `npm.cmd` via PATHEXT. On POSIX this is a no-op. The args (`view get-shit-done-cc version`) contain no spaces or shell metacharacters so quoting semantics are unaffected on all platforms. (#3103)
