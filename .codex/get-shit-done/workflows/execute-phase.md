...
**1. Check config:**
```bash
REQUIRE_TESTS=$(node ./.codex/get-shit-done/bin/gsd-tools.cjs config-get testing.require_passing_tests 2>/dev/null || echo "true")
TEST_CMD=$(node ./.codex/get-shit-done/bin/gsd-tools.cjs config-get testing.test_command 2>/dev/null || echo "")
```
...
If `--skip-tests` in {{GSD_ARGS}}: bypass test suite with warning:
```
