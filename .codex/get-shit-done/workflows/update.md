...
LOCAL_CONFIG="./.codex/gsd-install.json"
...
**Your fork location:** Check `./.codex/cache/gsd-update-check.json` for `fork_path`
```
...
**For fork mode:** If `./.codex/cache/gsd-update-check.json` contains `"mode": "fork"`, also update the per-project fork SHA:
```bash
