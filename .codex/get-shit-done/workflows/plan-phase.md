...
```bash
TDD_ENFORCEMENT=$(node ./.codex/get-shit-done/bin/gsd-tools.cjs config-get testing.tdd_enforcement 2>/dev/null || echo "true")
```
...
PHASE_GOAL=$(node ./.codex/get-shit-done/bin/gsd-tools.cjs roadmap get-phase "${PHASE}" | jq -r '.goal // .name')
