---
phase: 01-constitution-foundation
plan: 02
type: tdd
wave: 2
depends_on: ["01-01"]
files_modified:
  - package.json
  - ~/.claude/get-shit-done/src/constitution/loader.js
  - ~/.claude/get-shit-done/src/constitution/__tests__/loader.test.js
  - src/constitution/loader.js
  - src/constitution/__tests__/loader.test.js
autonomous: true

must_haves:
  truths:
    - "Loader loads global constitution from ~/.claude/get-shit-done/CONSTITUTION.md"
    - "Loader loads project constitution from .planning/CONSTITUTION.md"
    - "Loader merges global + project with project override precedence"
    - "Loader validates version compatibility (major version must match)"
    - "Loader validates rule ID uniqueness after merge"
    - "Loader tested in GSD install location before repo updated"
    - "Test suite achieves 100% coverage"
  artifacts:
    - path: "package.json"
      provides: "Dependencies: gray-matter, semver, deepmerge, mock-fs"
      contains: "gray-matter"
    - path: "~/.claude/get-shit-done/src/constitution/loader.js"
      provides: "ConstitutionLoader class (testing)"
      exports: ["ConstitutionLoader"]
      min_lines: 100
    - path: "src/constitution/loader.js"
      provides: "ConstitutionLoader class (repo source)"
      exports: ["ConstitutionLoader"]
      min_lines: 100
    - path: "~/.claude/get-shit-done/src/constitution/__tests__/loader.test.js"
      provides: "Tests for constitution loading (testing)"
      contains: "describe('ConstitutionLoader'"
      min_lines: 150
    - path: "src/constitution/__tests__/loader.test.js"
      provides: "Tests for constitution loading (repo source)"
      contains: "describe('ConstitutionLoader'"
      min_lines: 150
  key_links:
    - from: "src/constitution/__tests__/loader.test.js"
      to: "src/constitution/loader.js"
      via: "require statement"
      pattern: "require.*loader"
    - from: "src/constitution/loader.js"
      to: "gray-matter package"
      via: "require('gray-matter')"
      pattern: "require\\('gray-matter'\\)"
    - from: "src/constitution/loader.js"
      to: "global/project CONSTITUTION.md files"
      via: "fs.readFileSync with path.join"
      pattern: "readFileSync.*CONSTITUTION\\.md"
---

<objective>
Implement constitution loader using TDD approach. Test in GSD install location, then update repo source.

Purpose: Core module for constitutional enforcement system. Two-stage approach allows testing with actual GSD constitution files before committing to source. Follows red-green-refactor cycle with 100% coverage requirement.

Output: Working, tested constitution loader in GSD install and repo source.
</objective>

<execution_context>
@~/.claude/get-shit-done/workflows/execute-plan.md
@~/.claude/get-shit-done/templates/summary.md

**Two-stage approach (from RESEARCH.md):**
1. Implement and test in ~/.claude/get-shit-done/src/constitution/
2. Verify with actual GSD constitution files
3. Copy working implementation to repo source (src/constitution/)
4. Commit repo changes to git

**TDD Cycle Protocol:**
1. RED: Write failing test defining expected behavior
2. GREEN: Write minimal code to make test pass
3. REFACTOR: Clean up if needed (optional)
4. Commit after each cycle: test(01-02), feat(01-02), refactor(01-02)

**Coverage requirement:** 100% (statements, branches, functions, lines) - constitution code is critical infrastructure

Test execution: `npm test` (runs tests in watch mode or single run)
Coverage check: `npm test -- --coverage`
</execution_context>

<context>
@/Users/maciejmoszoro/Projects/GSD-with-constitution/get-shit-done-mm/.planning/PROJECT.md
@/Users/maciejmoszoro/Projects/GSD-with-constitution/get-shit-done-mm/.planning/ROADMAP.md
@/Users/maciejmoszoro/Projects/GSD-with-constitution/get-shit-done-mm/.planning/REQUIREMENTS.md
@/Users/maciejmoszoro/Projects/GSD-with-constitution/get-shit-done-mm/.planning/phases/01-constitution-foundation/01-CONTEXT.md
@/Users/maciejmoszoro/Projects/GSD-with-constitution/get-shit-done-mm/.planning/phases/01-constitution-foundation/01-RESEARCH.md
@/Users/maciejmoszoro/Projects/GSD-with-constitution/get-shit-done-mm/.planning/codebase/CONVENTIONS.md

Prior work:
@/Users/maciejmoszoro/Projects/GSD-with-constitution/get-shit-done-mm/.planning/phases/01-constitution-foundation/01-01-SUMMARY.md
</context>

<feature>
  <name>Constitution Loader</name>
  <files>
    - ~/.claude/get-shit-done/src/constitution/loader.js (testing)
    - ~/.claude/get-shit-done/src/constitution/__tests__/loader.test.js (testing)
    - src/constitution/loader.js (repo source)
    - src/constitution/__tests__/loader.test.js (repo source)
    - package.json (dependencies)
  </files>
  <behavior>
Expected behavior in testable terms:

**Load behavior:**
- `loader.load()` with both files → merged config
- `loader.load()` with global only → global config
- `loader.load()` with project only → project config
- `loader.load()` with neither → throws error

**Parse behavior:**
- Valid YAML frontmatter → extracted version, lastUpdated
- Valid markdown sections → extracted rules by severity
- Rule format `### RULE-ID: Description` → parsed to {id, description, severity}
- Version as quoted string "1.0" → remains string, not coerced to number

**Merge behavior:**
- Global + project rules → combined with project arrays replacing global arrays
- Matching rule IDs → project rule replaces global rule
- Different rule IDs → both included in result

**Version validation:**
- Global 1.0.0 + project 1.0.0 → passes
- Global 1.5.0 + project 1.2.0 → passes (same major)
- Global 2.0.0 + project 1.0.0 → throws error (major mismatch)
- Invalid semver → throws error
- Missing version field → throws error

**Rule ID validation:**
- Unique rule IDs → passes
- Duplicate rule ID (TDD-01 in both global and project after merge) → throws error

**Path handling:**
- Uses `path.join(os.homedir(), '.claude', 'get-shit-done', 'CONSTITUTION.md')` for global
- Uses `path.join(process.cwd(), '.planning', 'CONSTITUTION.md')` for project
- No string concatenation, no hardcoded `/` separators

**Test categories (MANDATORY - from RESEARCH.md):**
1. **Parsing tests** - YAML frontmatter, markdown sections, rule extraction
2. **Validation tests** - Version format, required fields, rule ID uniqueness
3. **Merging tests** - Global + project merge, array replacement, override precedence
4. **Error handling tests** - Missing files, parse errors, version conflicts
5. **Edge case tests** - Empty files, malformed YAML, duplicate rule IDs

**RED-GREEN-REFACTOR verification:**
After writing each test: `npm test` should show 1 failing test (RED)
After implementing feature: `npm test` should show all tests passing (GREEN)
  </behavior>
  <implementation>
Implementation approach after tests pass:

**Stage 1: Implement in GSD install location**

Create directory:
```bash
mkdir -p ~/.claude/get-shit-done/src/constitution/__tests__
```

Structure:
```javascript
class ConstitutionLoader {
  constructor() {
    this.globalPath = path.join(os.homedir(), '.claude', 'get-shit-done', 'CONSTITUTION.md');
    this.projectPath = path.join(process.cwd(), '.planning', 'CONSTITUTION.md');
  }

  load() {
    // Load files (sync during init)
    // Parse both
    // Validate versions
    // Merge with project precedence
    // Validate rule uniqueness
    // Return merged config
  }

  _readFile(filepath) { /* fs.existsSync + readFileSync */ }
  _parse(content) { /* gray-matter + regex rule extraction */ }
  _checkVersions(global, project) { /* semver validation */ }
  _merge(global, project) { /* deepmerge with array replacement */ }
  _validateRuleIds(config) { /* Set-based uniqueness check */ }
}
```

**Dependencies (add to package.json if missing):**
```json
"dependencies": {
  "gray-matter": "^4.0.3",
  "semver": "^7.5.4",
  "deepmerge": "^4.3.1"
},
"devDependencies": {
  "jest": "^29.0.0",
  "mock-fs": "^5.2.0"
}
```

**Testing with mock-fs (from RESEARCH.md):**
```javascript
const mock = require('mock-fs');

describe('ConstitutionLoader', () => {
  afterEach(() => {
    mock.restore(); // Restore real filesystem
  });

  test('loads global constitution when it exists', () => {
    mock({
      '/Users/testuser/.claude/get-shit-done/CONSTITUTION.md': `---
version: "1.0.0"
---

# NON-NEGOTIABLE

### TDD-01: Test before implementation
`
    });

    const loader = new ConstitutionLoader();
    const constitution = loader.load();

    expect(constitution.version).toBe('1.0.0');
    expect(constitution.rules['NON-NEGOTIABLE']).toHaveLength(1);
  });
});
```

**Anti-patterns to avoid (from RESEARCH.md):**
- NO async file loading (sync is appropriate during init)
- NO string concatenation for paths (use path.join)
- NO array concatenation for rule merging (use replacement strategy)
- NO manual version comparison (use semver.gt/lt)
- NO mutation of global config (deepmerge with clone: true)
- NO testing without mock-fs (creates cleanup issues, permission problems)

**Stage 2: Copy to repo source after tests pass**

After all tests passing in GSD install location:
```bash
mkdir -p src/constitution/__tests__
cp ~/.claude/get-shit-done/src/constitution/loader.js src/constitution/
cp ~/.claude/get-shit-done/src/constitution/__tests__/loader.test.js src/constitution/__tests__/
```

Follow RESEARCH.md code examples for complete implementation patterns.
  </implementation>
</feature>

<verification>
Stage 1 verification (GSD install location):
- TDD cycle completed for constitution loader feature
- All tests passing: `npm test` shows 0 failures
- Coverage at 100%: `npm test -- --coverage` confirms statements/branches/functions/lines all 100%
- Test file contains >= 150 lines covering all 5 mandatory test categories:
  1. Parsing tests (YAML frontmatter, markdown sections, rule extraction)
  2. Validation tests (version format, required fields, rule ID uniqueness)
  3. Merging tests (global + project merge, array replacement)
  4. Error handling tests (missing files, parse errors, version conflicts)
  5. Edge case tests (empty files, malformed YAML, duplicate IDs)
- Implementation file contains >= 100 lines with ConstitutionLoader class
- Dependencies installed in package.json (gray-matter, semver, deepmerge, mock-fs)
- Each RED-GREEN cycle committed separately
- Loader works with actual GSD constitution files from plan 01-01

Stage 2 verification (repo source):
- Files copied from GSD install to repo source
- Repo files match GSD install files (diff confirms)
- Tests still pass when run from repo location
- Files tracked in git, ready for commit
</verification>

<success_criteria>
- [ ] Test file created in GSD install with failing tests (RED phase)
- [ ] `npm test` shows failing tests before implementation
- [ ] Implementation created in GSD install making tests pass (GREEN phase)
- [ ] `npm test` shows all tests passing after implementation
- [ ] `npm test -- --coverage` shows 100% coverage (statements, branches, functions, lines)
- [ ] Refactored if needed (REFACTOR phase optional)
- [ ] Test suite covers all 5 mandatory test categories from RESEARCH.md
- [ ] ConstitutionLoader exports load() method returning merged constitution
- [ ] Loader uses mock-fs for all filesystem tests
- [ ] Loader tested with actual constitution files from plan 01-01
- [ ] Dependencies added: gray-matter, semver, deepmerge, mock-fs
- [ ] Files copied from GSD install to repo source
- [ ] Repo source files match GSD install files exactly
- [ ] Code committed following test(01-02), feat(01-02) pattern
</success_criteria>

<output>
After completion, create `/Users/maciejmoszoro/Projects/GSD-with-constitution/get-shit-done-mm/.planning/phases/01-constitution-foundation/01-02-SUMMARY.md`
</output>
