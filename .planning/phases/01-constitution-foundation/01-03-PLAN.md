...
Implementation (following RESEARCH.md synchronous file loading pattern):
- Use Node.js fs module (sync methods appropriate for setup operations)
- Use `path.join()` for all path operations (cross-platform - RESEARCH.md Pattern 6)
- Read template from `~/.claude/get-shit-done/templates/constitution/*.md`
- Replace placeholder values (lastUpdated with current date: `new Date().toISOString().split('T')[0]`)
...
function initGlobalConstitution(targetDir) {
  ...
  // Read template (sync OK during init - RESEARCH.md)
...
function initProjectConstitution(targetDir) {
  // Similar implementation for project template
}
...
Functions use `path.join()` for all path operations (no string concatenation).
Functions replace lastUpdated placeholder with current date in YYYY-MM-DD format.
Functions use synchronous fs methods (readFileSync, writeFileSync).
  </verify>
  <done>
...
**Note:** Use `sed -i ''` for macOS compatibility, or `sed -i` for Linux. Command should handle both if possible, or document platform requirements.
...
lastUpdated field populated with current date in YYYY-MM-DD format.
...
- Both constitutions have version "1.0.0" (quoted) and current lastUpdated date
...
- [ ] Both constitutions have version "1.0.0" (quoted string)
- [ ] lastUpdated fields populated with current date (YYYY-MM-DD format)
- [ ] Integration tested in actual GSD environment
