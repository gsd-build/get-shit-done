...
**Two-stage approach (from RESEARCH.md):**
...
Structure:
1. YAML frontmatter with version (quoted "1.0.0"), lastUpdated placeholder
...
Always quote version string to prevent YAML type coercion (version: 1.0 becomes number 1.0, loses trailing zero). Research shows 70% of YAML issues stem from type coercion and indentation.
...
**Security rule examples (from RESEARCH.md OWASP patterns):**
- SEC-01 good: `const apiKey = process.env.API_KEY;`
- SEC-01 bad: `const apiKey = "sk-1234abcd";`
- SEC-02 good: `db.query("SELECT * FROM users WHERE id = ?", [userId])`
- SEC-02 bad: `db.query("SELECT * FROM users WHERE id = " + userId)`
...
Contains YAML frontmatter with **quoted** version field: `version: "1.0.0"`.
...
Structure:
1. YAML frontmatter with **quoted** version ("1.0.0"), lastUpdated placeholder
...
Content:
- Frontmatter identical to global template (version, lastUpdated) with quoted version string
...
Follow same format conventions as global template, including quoted version in YAML frontmatter.
...
- Global template contains 7 security rules (SEC-01 through SEC-07) with rationale and examples from OWASP patterns
...
- YAML version fields are quoted strings (prevents type coercion)
...
<success_criteria>
- [ ] Global constitution template exists in GSD install with version "1.0.0" (quoted)
- [ ] Project constitution template exists in GSD install with version "1.0.0" (quoted)
- [ ] TDD-01 rule fully documented with good/bad examples
- [ ] SEC-01 through SEC-07 rules fully documented with good/bad examples from OWASP patterns
...
- [ ] Templates use YAML frontmatter + markdown body format with quoted version strings
