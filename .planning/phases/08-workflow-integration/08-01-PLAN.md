...
4. **Per-agent loop** (~30 lines): For each agent in the agents array:
   ...
   e. Write the constructed prompt to a temp file to avoid shell quoting/escaping issues with multi-line artifact content:
      ```bash
      PROMPT_FILE=$(mktemp)
      cat > "$PROMPT_FILE" << 'PROMPT_EOF'
      {review prompt text with embedded artifact content}
      PROMPT_EOF
      RESULT=$(node ~/.claude/get-shit-done/bin/gsd-tools.cjs coplanner invoke {agent} --prompt "$(cat "$PROMPT_FILE")")
      rm -f "$PROMPT_FILE"
      ```
      This avoids passing raw REQUIREMENTS.md content (which may contain quotes, backticks, dollar signs) directly as a shell argument.
   f. Parse result JSON -- check for errorType

5. **Error handling per agent** (~5 lines): If errorType is non-null, display warning `{DISPLAY_NAME} failed ({errorType}): {error}` and continue to next agent. If ALL agents fail, display warning and skip to Phase 7.5.

6. **Feedback display** (~15 lines): For each successful agent response, display a bordered attributed block using the display name (not the raw CLI adapter name). Map adapter names to display names: `codex` -> "Codex", `gemini` -> "Gemini CLI", `opencode` -> "OpenCode". Use title case of the CLI name as fallback for any future adapters.
   ```
   --- {Display Name} Feedback ---
   (Claude parses response text into Suggestions, Challenges, Endorsements)
   ---
   ```
   Use `---` bordered sections with display name header (e.g., `--- Codex Feedback ---`).

7. **Synthesis** (~15 lines): Review all suggestions and challenges using the acceptance criteria below. Apply accepted changes to REQUIREMENTS.md via Write tool (read current content, apply changes, write back -- Edit is NOT in this command's allowed-tools). Set `ARTIFACT_REVISED=true` if any changes were made. Track what was changed.

   **Acceptance criteria for co-planner feedback** (parallel to adversary severity framework):
...
Specifically, the following patterns carry over unchanged from Task 1 and MUST be replicated:
- Temp file approach for prompt construction (step 4e -- avoids shell injection)
- Write tool for artifact modification (NOT Edit tool -- not in allowed-tools)
- Display name mapping for agent attribution (`codex` -> "Codex", etc.)
- Acceptance criteria framework (accept/reject/note with same thresholds as Task 1)
- Same three-section response format (Suggestions, Challenges, Endorsements)
