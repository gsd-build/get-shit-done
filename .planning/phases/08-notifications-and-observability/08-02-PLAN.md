...
<task type="checkpoint:human-verify" gate="blocking">
  <what-built>Voice message transcription via local Whisper model</what-built>
  <how-to-verify>
...
2. Download Whisper model:
   ```bash
   cd get-shit-done && npx whisper-node download
   ```

3. Start the bot:
   ```bash
   export TELEGRAM_BOT_TOKEN="your_token"
   export TELEGRAM_OWNER_ID="your_chat_id"
   node get-shit-done/bin/gsd-tools.js telegram start
   ```

4. In another terminal, send a blocking question:
   ```bash
   node get-shit-done/bin/gsd-tools.js telegram ask "Please respond by voice"
   ```

5. In Telegram, record a voice message (hold microphone button, speak, release)

6. Observe:
   - Bot replies "Processing voice message..."
   - Bot replies with transcription: "Transcribed: [your words]"
   - CLI shows the transcribed response

Expected: Voice message transcribed locally, text returned to conversation flow
  </how-to-verify>
  <resume-signal>Type "approved" if voice transcription works, or describe issues</resume-signal>
</task>
