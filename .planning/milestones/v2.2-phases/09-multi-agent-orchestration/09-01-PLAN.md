...
7. Apply `options.timeout` to the `exec` call's `timeout` option for per-agent timeout isolation. Also set `maxBuffer: 10 * 1024 * 1024` on the `exec` options — this matches the existing sync adapters and prevents `ERR_CHILD_PROCESS_STDIO_MAXBUFFER` errors or silent truncation on large agent responses.
...
- Per-agent timeout isolation: each `exec` call has its own timeout option and `maxBuffer: 10 * 1024 * 1024`
