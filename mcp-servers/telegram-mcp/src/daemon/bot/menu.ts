 ...
 */
export function formatStatusPanel(sessions: Session[]): StatusPanelResult {
  let text: string;
 ...
 * @returns Formatted text and inline keyboard
 */
export function formatQuestionsPanel(
  questions: Question[],
  sessions: Session[]
): QuestionsPanelResult {
  // Build a lookup map for session labels
  const sessionLabels = new Map<string, string>(
    sessions.map((s) => [s.id, s.label])
  );
