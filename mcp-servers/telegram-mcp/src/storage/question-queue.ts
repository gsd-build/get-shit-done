...
export interface PendingQuestion {
  type?: string;           // Entry type for JSONL (internal)
  id: string;              // UUID
 ...
 */
export async function loadPendingQuestions(sessionId: string): Promise<PendingQuestion[]> {
  const sessionPath = getSessionPath(sessionId);
  const entries = await loadSessionJSONL(sessionPath);

  return entries
    .filter((entry: any) => entry.type === 'question' && entry.status === 'pending')
 ...
 */
export async function loadAllPendingQuestions(): Promise<Array<PendingQuestion & { session_id: string }>> {
  const sessions = await discoverSessions();
  const allPending: Array<PendingQuestion & { session_id: string }> = [];
 ...
 */
export async function markAnswered(sessionId: string, questionId: string, answer: string): Promise<void> {
  const sessionPath = getSessionPath(sessionId);
 ...
 */
export async function getPendingById(questionId: string, sessionId?: string): Promise<PendingQuestion | null> {
  if (sessionId) {
 ...
 */
export async function loadPendingQuestionsLegacy(): Promise<PendingQuestion[]> {
 ...
 * @deprecated Will be removed in 10.1-03
 */
export async function appendQuestionLegacy(
 ...
 * @deprecated Will be removed in 10.1-03
 */
export async function markAnsweredLegacy(questionId: string, answer: string): Promise<void> {
