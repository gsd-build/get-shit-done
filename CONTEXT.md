...
Module owning the per-runtime mapping from artifact kind to filesystem placement. For each runtime (canonical enum supplied by `runtime-homes.cjs`), exposes the typed `kinds` that runtime materializes on disk — `commands` (Gemini-style flat `commands/gsd/`), `agents`, `skills` (most runtimes use `skills/gsd-<stem>/SKILL.md`; Hermes nests under `skills/gsd/<stem>/` per #2841; Cline omits the kind entirely). Each kind record carries its destination subpath, file/dir prefix, and stage function — the per-runtime skill converters in `bin/install.js` (`convertClaudeCommandToClaudeSkill`, `…CodexSkill`, `…CopilotSkill`, `…AntigravitySkill`) survive as adapters at this seam. Consumed by the install pipeline, the uninstall pipeline, and the Runtime Surface Module (`surface.cjs:applySurface`) — all three iterate the same typed kinds collection rather than re-encoding per-runtime layout knowledge. Closes the bug class where `applySurface` forgets an artifact kind that install/uninstall track (#3659). Migrations stay with the Installer Migration Module (ADR-0008) and run before/after layout-driven copy. See ADR-3660.
